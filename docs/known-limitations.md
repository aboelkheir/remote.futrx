...
- **No backup or restore mechanism.** Nothing snapshots, exports, or restores `DATA_DIR`, the workspaces at `/var/lib/remote/projects`, browser state at `/var/lib/remote/browser-broker`, its key under `/etc/remote.futrx`, or the containers. The only recovery-adjacent feature is the per-project Git "safety checkpoint," which protects code the agent edits — not chats, users, secrets, sessions, or container state. Disaster recovery is entirely the operator's responsibility, and a hand-rolled backup taken while services run can capture a torn view. **Snapshot the host or copy all of those paths yourself, ideally with both Remote services stopped.**
...
- **No password reset flow.** Recovering a lost owner password requires manually deleting `local-admin.json` on the host, then running `remote setup-token` to print a fresh setup link (deleting the credential alone leaves the claim gated).
...
- **One shared browser session per project.** The user and the agent drive the same 1280×720 BrowserContext — there is no per-agent or per-task browser isolation. Projects have separate contexts and encrypted storage state, but those contexts still share one Chromium multi-process instance and are not a VM-grade hostile-code boundary. Idle reaping counts an attached live-view WebSocket as a viewer.
- **Host-file browser tools are disabled.** File upload/drop, unsafe server-side Playwright code, and client-selected output paths are not exposed by the shared broker because remote MCP clients must not read or overwrite the host service account's files. Broker-generated screenshots are bounded and kept in its ephemeral runtime directory. Browser file upload is currently unavailable in pooled mode; use a site's API or another transfer path when possible.
- **The pooled human view is page-only.** Its canvas does not expose Chromium's native window chrome, permission bubbles, certificate prompts, or other browser-owned dialogs. Ordinary web login and OAuth pages work, but flows that require native browser UI may need a separate browser process.
- **A normal browser launch is not an anti-bot guarantee.** Chromium is launched directly rather than with Playwright's automation launch arguments, and the canvas viewer is outside the visited page. Agent actions still arrive through CDP, all contexts share the host's network/browser fingerprint, and sites can reject automation or unusual behavior. Do not treat the browser as a CAPTCHA or access-control bypass.
- **Persistence is web-session state, not a full Chrome user-data directory.** Cookies, local storage, and IndexedDB are restored. Cache, browsing history, downloads, extensions, and Chrome-level preferences are not promised across a context restart.
