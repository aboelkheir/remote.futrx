...
import type { AgentQuota } from "../../../models/agentQuota.ts";
...
export function startPlanQuotaUpdates({
  ...
  load: (signal: AbortSignal) => Promise<AgentQuota[]>;
  onSnapshot: (quotas: AgentQuota[]) => void;
  onSettled: () => void;
  ...
  async function refresh() {
    ...
    try {
      ...
      // A missed refresh must not erase the last successful provider reading.
    } finally {
